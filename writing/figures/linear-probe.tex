...
% Phase label
\node[phase] at (3.0, 2.6) {RL training};
...
% Trainable label
\node[font=\scriptsize, text=black!45] at (3.5, -3.4, 0)
    {weights updated via backpropagation};

% --- Freeze arrow ---
\draw[->, very thick, black!25]
    (8.0, 0) -- (9.4, 0)
    node[midway, above, lbl] {freeze};

% --- Phase 2: Frozen (simplified 2D blocks) ---
\node[phase] at (12.5, 2.6) {Evaluation};

\def\fy{0}

% Faded 2D blocks with varying height
\fill[encfade, draw=black!20, thick, rounded corners=1pt]
    (9.8, -1.2) rectangle (10.15, 1.2);
\fill[encfade, draw=black!20, thick, rounded corners=1pt]
    (10.55, -0.8) rectangle (11.0, 0.8);
\fill[encfade, draw=black!20, thick, rounded corners=1pt]
    (11.4, -0.65) rectangle (11.85, 0.65);
\fill[encfade, draw=black!20, thick, rounded corners=1pt]
    (12.25, -1.0) rectangle (12.38, 1.0);

\draw[arr, black!25] (10.15, 0) -- (10.55, 0);
\draw[arr, black!25] (11.0, 0) -- (11.4, 0);
\draw[arr, black!25] (11.85, 0) -- (12.25, 0);

\node[lbl, text=black!35] at (11.1, -2.0) {weights fixed};

% h_t output
\draw[arr] (12.38, 0) -- (13.3, 0);
\node[draw=black!40, thick, fill=outfront, rounded corners=2pt,
      font=\small, inner sep=5pt] (ht) at (13.7, 0) {$h_t$};

\end{tikzpicture}%
}
\caption{The encoder is trained end-to-end via RL, then its weights
  are frozen. The fixed encoder produces representations $h_t$ for
...
% Frozen encoder (faded 2D blocks)
\node[lbl] at (-0.1, 0) [left] {$s_t$};
\draw[arr, black!25] (0.0, 0) -- (0.5, 0);

\fill[encfade, draw=black!20, thick, rounded corners=1pt]
    (0.5, -1.2) rectangle (0.85, 1.2);
\fill[encfade, draw=black!20, thick, rounded corners=1pt]
    (1.25, -0.8) rectangle (1.7, 0.8);
\fill[encfade, draw=black!20, thick, rounded corners=1pt]
    (2.1, -0.65) rectangle (2.55, 0.65);
\fill[encfade, draw=black!20, thick, rounded corners=1pt]
    (2.95, -1.0) rectangle (3.08, 1.0);

\draw[arr, black!25] (0.85, 0) -- (1.25, 0);
\draw[arr, black!25] (1.7, 0) -- (2.1, 0);
\draw[arr, black!25] (2.55, 0) -- (2.95, 0);

% Frozen brace
\draw[decorate, decoration={brace, amplitude=3pt, mirror}, black!25]
    (0.5, -1.5) -- (3.08, -1.5)
    node[midway, below=4pt, lbl, text=black!30] {frozen};

% h_t
\draw[arr] (3.08, 0) -- (3.9, 0);
...
\caption{Linear probe methodology. (a)~The encoder is trained via
  RL, then frozen. (b)~A linear layer predicts game-state properties
  from frozen representations. (c)~Probes at multiple layers measure
  how information is organized across the representation hierarchy.}

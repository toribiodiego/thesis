...
Three parameters differ from the SPR paper's data-efficient Rainbow
and are likely responsible for the performance gap:

\medskip
\textbf{1. Replay ratio: 0.25 vs.\ 2.0 (8$\times$ fewer gradient
steps).} Our config performs one gradient update per 4 environment
steps. The SPR paper performs 2 updates per 1 step. Over 100K
environment steps, SPR performs $\sim$200K gradient updates; we
perform $\sim$25K. This is the single largest lever. The training
scheduler (\texttt{src/training/schedulers.py}) currently only
supports a single update per trigger; implementing replay ratio $>1$
requires a loop.

Source: Schwarzer et al.\ (2021), Table~3: ``replay ratio = 2''.

\medskip
\textbf{2. Multi-step $n$: 3 vs.\ 10.} We use the original Rainbow
...
\medskip
\textbf{3. Target network: hard copy every 8K steps vs.\ every 1
...
keeping targets fresh. With 200K gradient steps, even per-step hard
copies are practical.
...
\begin{itemize}[leftmargin=2em, itemsep=0.3em]
  ...
  \item \textbf{FC hidden dim}: 512 vs.\ 256 (larger network may
    ...
    replay ratio 2).
  \item \textbf{Warmup}: 5000 transitions (20K frames) vs.\ 2000
    transitions (8K frames).
  \item \textbf{PER $\beta$ start}: 0.4 vs.\ 0.5 (minor effect on
    IS correction early in training).
\end{itemize}
...
We fix discrepancies one at a time and re-run on 2 validation games
(Crazy Climber and Up N Down -- chosen for large published-vs-ours
gap and dense rewards) to measure the delta.

\medskip
\begin{enumerate}[leftmargin=2em, itemsep=0.5em]
  \item \textbf{Replay ratio}: Add \texttt{updates\_per\_step}
    config parameter to the training scheduler. Set to 2 for Rainbow
    configs. This requires a loop in the training step logic.
  \item \textbf{Multi-step $n$}: Change from 3 to 10 in Rainbow
    configs.
  \item \textbf{Target update}: Change from 8000 to 1 in Rainbow
    configs.
  \item \textbf{Learning rate}: Change from 6.25e-5 to 1.0e-4 in
    Rainbow configs.
  \item \textbf{Warmup}: Reduce from 20K frames to 8K frames (2K
    transitions) in Rainbow configs.
  \item \textbf{FC hidden}: Reduce from 512 to 256 in Rainbow model
    (requires RainbowDQN architecture change).
\end{enumerate}

Steps 1--3 are high-impact and will be applied together in the first
validation run. Steps 4--6 will be applied individually to isolate
their effects.
...
\subsection{Fix 1: Replay ratio + multi-step + target update}

\noindent\textit{Pending.}\\
\noindent Changes: \texttt{updates\_per\_step = 8} (with
\texttt{train\_every = 4}, effective replay ratio = 2.0),
\texttt{multi\_step.n = 10}, \texttt{target\_update = 1}.
...
\subsection{Fix 3: Warmup reduction}

\noindent\textit{Pending.}\\
\noindent Change: \texttt{warmup\_steps = 2000}, \texttt{min\_size = 2000}.

\subsection{Fix 4: FC hidden dimension}
...
\subsection{Evaluation Protocol Note}

The Atari-100K benchmark (Kaiser et al.\
\citelink{https://arxiv.org/abs/1903.00374}{4}) specifies 100
evaluation episodes with $\epsilon = 0.001$. Our current DQN config
uses 30 episodes at $\epsilon = 0.05$. This should be aligned before
final batch runs to ensure comparability with published numbers.

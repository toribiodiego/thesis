...
def base_config_file(temp_config_dir):
    ...
    base_config = {
        "experiment": {"name": "base_experiment", "seed": 42},
        "training": {
            "lr": 0.001,
            "gamma": 0.99,
            "optimizer": {"type": "adam", "eps": 1e-8},
        },
        "network": {"hidden": 256},
    }
...
def test_deep_merge_deep_nesting():
    ...
    base = {"level1": {"level2": {"level3": {"a": 1, "b": 2}, "x": 10}}}
    override = {"level1": {"level2": {"level3": {"b": 99}}}}
...
def test_apply_overrides_multiple_nested():
    ...
    config = {"training": {"lr": 0.001, "optimizer": {"type": "adam", "eps": 1e-8}}}
    overrides = {"training.lr": 0.0001, "training.optimizer.eps": 1e-2}
...
def test_load_config_with_overrides(game_config_file):
    ...
    overrides = {"experiment.seed": 123, "training.gamma": 0.95}
...
def test_load_config_nested_overrides(game_config_file):
    ...
    overrides = {"training.optimizer.type": "rmsprop", "training.optimizer.eps": 1e-2}
...
def test_full_workflow_with_real_configs(temp_config_dir):
    ...
    base_config = {
        ...
        "training": {"lr": 0.001, "gamma": 0.99, "optimizer": {"type": "adam"}},
    }
    base_file = temp_config_dir / "base.yaml"
    with open(base_file, "w") as f:
        yaml.dump(base_config, f)

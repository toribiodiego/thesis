...
def sample_eval_csv(temp_dir):
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["step", "mean_return", "std_return", "num_episodes"]
        )
        writer.writeheader()
...
def test_plot_episode_returns(sample_episodes_csv, temp_dir):
    ...
    plot_episode_returns(data, output_path, smoothing_window=10, formats=["png"])
...
def test_plot_training_loss(sample_steps_csv, temp_dir):
    ...
    plot_training_loss(data, output_path, smoothing_window=10, formats=["png"])
...
def test_plot_evaluation_scores(sample_eval_csv, temp_dir):
    ...
    plot_evaluation_scores(data, output_path, formats=["png"])
...
def test_plot_epsilon_schedule(sample_steps_csv, temp_dir):
    ...
    plot_epsilon_schedule(data, output_path, formats=["png"])
...
def test_plot_multiple_formats(sample_episodes_csv, temp_dir):
    ...
    plot_episode_returns(data, output_path, formats=["png", "pdf", "svg"])
...
def test_plot_all_metrics(
    sample_episodes_csv, sample_steps_csv, sample_eval_csv, temp_dir
):
...
def test_plot_with_nan_values(temp_dir):
    ...
    plot_episode_returns(data, output_path, smoothing_window=2, formats=["png"])
...
def test_plot_empty_data_warning(temp_dir, capsys):
    ...
    plot_episode_returns(data, output_path, formats=["png"])
...
def test_plot_directory_creation(temp_dir):
    ...
    data = {"step": np.arange(100) * 1000, "return": np.random.randn(100) + 10.0}

...
\end{tikzpicture}
\caption{Structured representation. Points with the same game-state
  property (color) form distinct clusters, indicating the encoder
  organizes states by task-relevant features.}
...
\end{tikzpicture}
\caption{Unstructured representation. The same coloring reveals no
  grouping -- game-state properties are not linearly separable in
  the projected space.}
...
\caption{Interpreting latent space visualizations. Each point is a
  ...
  colored by a game-state property. (a)~A structured representation
  groups similar states together. (b)~An unstructured representation
  shows no relationship between proximity and game-state similarity.
  Actual results may fall between these extremes.}
\label{fig:viz-guide}
\end{figure}

...
\bottomrule
\end{tabular}
\caption{Vanilla DQN baseline at 100K env steps in clipped reward space.
30 eval episodes per checkpoint, eval epsilon 0.05.}
\label{tab:baseline-clipped}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Vanilla DQN baseline on the six selected games with raw
(unclipped) evaluation scores. Random and human scores from Schwarzer
et al.\ \citelink{https://arxiv.org/abs/2007.05929}{3}.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Effect of random-shift data augmentation compared to the
vanilla DQN baseline. Seed 42, 30 eval episodes at 400K frames.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Effect of self-predictive representations compared to the
vanilla DQN baseline. Seed 42, 30 eval episodes at 400K frames.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{All conditions. Mean raw episode return ($\pm$ std) over 30
eval episodes, seed 42.}
\end{table}
...
The initial baseline validation (Table~\ref{tab:baseline-clipped}) reported returns in
...
runs (Table~\ref{tab:baseline-clipped} onward) by disabling reward clipping in the evaluation
environment.
...
(Table~\ref{tab:baseline-clipped}):

...
Table~\ref{tab:rainbow} compares vanilla DQN against Rainbow DQN on
the same six games. Rainbow dramatically improves two games while
performing comparably or worse on the remaining four.
...
\textbf{Game}
  ...
  & \textbf{$\Delta$} \\
\midrule
Crazy Climber  & $3540 \pm 754$   & $9137 \pm 3857$   & $+158\%$  \\
Up N Down      & $141 \pm 155$    & $1794 \pm 637$    & $+1172\%$ \\
Road Runner    & $1440 \pm 859$   & $180 \pm 464$     & $-88\%$   \\
Frostbite      & $147 \pm 45$     & $43 \pm 18$       & $-71\%$   \\
Kangaroo       & $26.7 \pm 68.0$  & $33.3 \pm 90.7$   & $+25\%$   \\
Boxing         & $-3.0 \pm 5.3$   & $-3.0 \pm 6.5$    & $\approx 0$ \\
\midrule
Mean           & $982$  & $1864$  & $+90\%$ \\
\bottomrule
\end{tabular}
\caption{Rainbow baseline results. Mean raw episode return ($\pm$ std)
over 30 eval episodes at 400K frames, seed 42. Games sorted by
Rainbow's relative improvement over vanilla DQN.}
\label{tab:rainbow}
\end{table}

\noindent Crazy Climber and Up N Down see large gains under Rainbow --
nearly $3\times$ and $12\times$ respectively. These are the two games
where the DQN isolation study also showed the strongest SPR and
augmentation effects, suggesting that dense reward games with
relatively simple visual dynamics benefit most from additional agent
capability under the 100K budget.

\medskip
\noindent Road Runner and Frostbite decline substantially. Both games
performed reasonably under vanilla DQN, so the regression is not
simply a floor effect. One possible explanation: Rainbow's additional
components (prioritized replay warmup of 20K steps, distributional
targets across 51 atoms, noisy exploration) require more data to
calibrate than the 100K budget provides. Vanilla DQN's simpler
learning signal may be easier to fit with limited experience.

\medskip
\noindent Boxing and Kangaroo are effectively unchanged -- both agents
score near zero. These games appear too difficult for either agent
to make progress within 100K steps.

\medskip
\noindent The overall picture is that Rainbow is not uniformly better
under severe data constraints. Its advantage is concentrated in games
where its components align with the reward structure. This sets up
the key comparison: does adding SPR to Rainbow amplify gains in the
games where Rainbow already helps, and can it rescue the games where
Rainbow alone regresses?

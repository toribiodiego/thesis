...
$12\times$) see large gains, while Road Runner and Frostbite regress
substantially. Boxing and Kangaroo are unchanged -- both agents score
near zero.
...
near zero with high variance. Boxing shows an interesting divergence:
Rainbow recovers to near-zero while DQN declines steadily.}
\label{fig:rainbow-flat}
\end{figure}

\noindent The advantage appears concentrated in dense reward games where
Rainbow's components (distributional targets, prioritized replay,
n-step returns) can calibrate within 100K steps. In other games, the
added complexity may hurt more than it helps with so little data. This
sets up the key comparison: does adding SPR to Rainbow amplify gains in
the games where Rainbow already helps, and can it rescue the games
where Rainbow alone regresses?

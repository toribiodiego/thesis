# DQN Experiment Log

| Date | Run / Config | Observations | Follow-Ups |
|------|--------------|--------------|------------|
| YYYY-MM-DD | `pong_seed0` | Example: Loss spikes around 1M frames; need gradient clipping. | [ ] Investigate RMSProp hyperparameters |

Guidelines:
- Append rows chronologically; keep descriptions short but actionable.
- Link to relevant configs, commits, or paper notes.
- Flag urgent items for follow-up outside this file.

...
\caption{A grid-world MDP\@. An agent (blue circle) navigates a
  $4 \times 3$ grid from a starting cell toward a goal ($+1$)
  while avoiding a penalty ($-1$). (a)~Each accessible cell is a
  state, labeled by column and row; the shaded wall is impassable,
  giving eleven states in total. (b)~At each step the agent

...
def test_tensorboard_backend_log_scalars(temp_log_dir):
    ...
    if backend.enabled:
        metrics = {"train/loss": 0.5, "train/epsilon": 0.95, "train/fps": 120.0}
...
def test_csv_backend_log_step_metrics(temp_log_dir):
    ...
    metrics = {"loss": 0.5, "epsilon": 0.95, "fps": 120.0}
...
def test_csv_backend_log_episode_metrics(temp_log_dir):
    ...
    metrics = {"return": 21.0, "length": 1200, "return_ma": 18.5}
...
def test_metrics_logger_log_q_values(temp_log_dir):
    ...
    logger.log_q_values(step=1000, q_mean=5.2, q_std=1.8, q_min=2.1, q_max=8.9)
...
def test_metrics_logger_extra_metrics(temp_log_dir):
    ...
    extra = {"custom_metric_1": 42.0, "custom_metric_2": 123.4}

    # Should not raise error even though CSV doesn't support extra metrics
    logger.log_step(step=1000, loss=0.5, extra_metrics=extra)
...
def test_metrics_logger_multi_backend_consistency(temp_log_dir):
    ...
    # Log step metrics
    logger.log_step(step=1000, loss=0.5, epsilon=0.95)
...
def test_metrics_logger_periodic_flush(temp_log_dir):
    ...
    logger = MetricsLogger(log_dir=temp_log_dir, enable_csv=True, flush_interval=1000)
...
def test_metrics_logger_deterministic_artifact_name(temp_log_dir):
    ...
    logger = MetricsLogger(
        log_dir=temp_log_dir, enable_csv=True, enable_wandb=False, upload_artifacts=True
    )

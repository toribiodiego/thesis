...
def valid_config():
    ...
    return {
        ...
        "environment": {
            ...
            "action_repeat": 4,
            "preprocessing": {"frame_size": 84, "frame_stack": 4, "grayscale": True},
        },
        "network": {
            "architecture": "dqn",
            "conv1_channels": 16,
        ...
        "target_network": {"update_interval": 10000, "update_method": "hard"},
        "exploration": {
            "schedule": {
                "type": "linear",
                "start_epsilon": 1.0,
        ...
        "logging": {
            ...
            "checkpoint": {"enabled": True, "save_every": 1000000, "keep_last_n": 3},
        },
        "seed": {"value": 42},
        "system": {"num_workers": 0, "empty_cache_every": 1000, "progress_bar": True},
    }
...
def test_environment_invalid_env_id():
    ...
    with pytest.raises(
        ConfigValidationError, match="unknown environment 'InvalidGame-v4'"
    ):
        validate_environment(config)
...
def test_unknown_field_nested():
    ...
    config = {
        ...
        "training": {
            "gamma": 0.99,
            "optimizer": {"type": "rmsprop", "unknown_opt": "value"},
        },
    }
...
def test_validate_config_invalid_env_id():
    ...
    config = {"experiment": {"name": "test"}, "environment": {"env_id": "Tetris-v0"}}
    with pytest.raises(ConfigValidationError) as exc_info:
        validate_config(config)
    error_msg = str(exc_info.value)

...
class SimpleMockEnv:
    ...
    def __init__(self, obs_shape=(4, 84, 84), num_actions=6, seed=42):
        ...
        self.observation_space = type("ObservationSpace", (), {"shape": obs_shape})()
    ...
    for step in range(start_step, start_step + num_steps):
        ...
        with torch.no_grad():
            action = select_epsilon_greedy_action(
                online_model, state.squeeze(0), epsilon, env.num_actions
            )
        ...
        replay_buffer.append(obs, action, reward, next_obs, done or truncated)
...
def compute_checksum(
    epsilons: List[float], rewards: List[float], actions: List[int]
) -> str:
...
def compare_metrics(
    baseline: Dict[str, Any], resumed: Dict[str, Any], tolerance: float = 1e-5
) -> Dict[str, Any]:
    """
...
def print_comparison_report(report: Dict[str, Any]):
    ...
    print("\nEpsilon Values:")
    print(
        f"  Matches:    {report['epsilon_matches']:5d} / {total} ({report['epsilon_matches']/total*100:.1f}%)"
    )
    print(
        f"  Mismatches: {report['epsilon_mismatches']:5d} / {total} ({report['epsilon_mismatches']/total*100:.1f}%)"
    )
    print(f"  Max Diff:   {report['max_epsilon_diff']:.2e}")

    print("\nRewards:")
    print(
        f"  Matches:    {report['reward_matches']:5d} / {total} ({report['reward_matches']/total*100:.1f}%)"
    )
    print(
        f"  Mismatches: {report['reward_mismatches']:5d} / {total} ({report['reward_mismatches']/total*100:.1f}%)"
    )
    print(f"  Max Diff:   {report['max_reward_diff']:.2e}")

    print("\nActions:")
    print(
        f"  Matches:    {report['action_matches']:5d} / {total} ({report['action_matches']/total*100:.1f}%)"
    )
    print(
        f"  Mismatches: {report['action_mismatches']:5d} / {total} ({report['action_mismatches']/total*100:.1f}%)"
    )
    ...
    all_match = (
        ...
        and report["epsilon_mismatches"] == 0
        and report["reward_mismatches"] == 0
        and report["action_mismatches"] == 0
    )

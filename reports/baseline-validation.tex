...
\subsection{Fix 1: Sticky actions}

\noindent\textit{Pending.}

\noindent Changes:
\begin{itemize}[leftmargin=2em, itemsep=0.1em]
  \item Add \texttt{repeat\_action\_probability = 0.25} to
    environment config (Machado et al.\ 2018)
  \item Pass parameter through \texttt{make\_atari\_env} to
    \texttt{gym.make}
\end{itemize}

\noindent\textit{Impact:} All 24 DQN validation runs (4 conditions
$\times$ 6 games, seed 42) must be re-run. Previous runs moved to
\texttt{invalid/} directory. Rainbow validation runs also affected.

\subsection{Fix 2: Training cadence + replay ratio + multi-step +
target update + warmup (Rainbow only)}
...
\subsection{Fix 3: Learning rate (Rainbow only)}

\noindent\textit{Pending.}\\
\noindent Change: \texttt{lr = 1.0e-4}.

\subsection{Fix 4: FC hidden dimension (Rainbow only)}

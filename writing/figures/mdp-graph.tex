...
% Direction arrows with gap from dot, ending near cell edge
\draw[->, semithick, blue!65]
      ([yshift=7pt]c21.center) -- ([yshift=14pt]c21.center);
\draw[->, semithick, blue!65]
      ([xshift=7pt]c22.center) -- ([xshift=14pt]c22.center);
...
\caption{Two views of the same MDP fragment. Left: the agent
  follows a path through the grid-world from $s_{21}$ to the
  goal~$s_{32}$. Right: the same states and transitions as a
  directed graph. Each arrow is one entry of the dynamics
  function $p(s', r \mid s, a)$: the action label gives~$a$,
  the destination gives~$s'$, and the annotation gives~$r$.
  Highlighted edges are the path taken; faded edges are
  alternatives. Double-bordered nodes are terminal.}
\label{fig:mdp-views}
\end{figure}
